...
void Editor::MapEdit()
{
	if(mouseInMapArea)
	{
		...
		// 타일 영역의 중앙 좌표 계산
		int centerX = mapArea.left + tileX * TILE_SIZE + (TILE_SIZE / 2);
		int centerY = mapArea.top + tileY * TILE_SIZE + (TILE_SIZE / 2);

		// GetRectAtCenter로 타일 영역 계산
		RECT tileRect = GetRectAtCenter(centerX,centerY,TILE_SIZE,TILE_SIZE);
...
void Editor::Shortcut()
{
	...
	else if(KeyManager::GetInstance()->IsOnceKeyDown('5'))
	{
	...
	else if(KeyManager::GetInstance()->IsOnceKeyDown('C'))
	{
		...
		{
			if(MessageBox(g_hWnd,TEXT("맵을 초기화하시겠습니까?"),TEXT("확인"),MB_YESNO) == IDYES)
			{
				ClearMap();
			}
		}
...
void Editor::Render(HDC hdc)
{
	...
	RenderTiles(hdc);
	RenderSprites(hdc);
	RenderObstacles(hdc); // 이 부분이 추가되어야 함
	...
	if(mode == EditorMode::TILE) {
		...
		modeName = TEXT("MONSTER MODE (place obstacle)");
		modeColor = RGB(255,100,100);
	}
	...
	RECT guideRect = {0,WINSIZE_Y - 400,WINSIZE_X,WINSIZE_Y};
	...
	TextOut(hdc,20,WINSIZE_Y - 30,TEXT("1-4: Change Mode, S: Save, L: Load"),
			lstrlen(TEXT("1-5: Change Mode, S: Save, L: Load")));

	SelectObject(hdc,oldFont);
	DeleteObject(guideFont);
}

void Editor::RenderTiles(HDC hdc)
{
	// 맵 섹션 제목 추가
	...
	for(int y = 0; y < VISIBLE_MAP_HEIGHT; y++)
	{
		for(int x = 0; x < VISIBLE_MAP_WIDTH; x++)
		{
			...
			{
				// GetRectAtCenter 활용하여 중앙 기준 사각형 생성
				RECT startRect = GetRectAtCenter(screenX,screenY,tileSize / 2,tileSize / 2);

				HBRUSH greenBrush = CreateSolidBrush(RGB(0,255,0));
				HPEN greenPen = CreatePen(PS_SOLID,3,RGB(0,200,0));
			...
			if(mouseInMapArea &&
			   x == (mousePos.x - mapArea.left) / tileSize &&
			   y == (mousePos.y - mapArea.top) / tileSize)
			{
				RECT hoverRect = GetRectAtCenter(screenX,screenY,tileSize,tileSize);
				HPEN hoverPen = CreatePen(PS_DOT,1,RGB(255,255,0)); // 노란색 점선
...
void Editor::RenderSampleTiles(HDC hdc)
{
	if(sampleTileImage)
	{
		int sampleTileSize = (sampleArea.right - sampleArea.left) / SAMPLE_TILE_X;
...
void Editor::RenderSprites(HDC hdc)
{
...
void Editor::RenderObstacles(HDC hdc)
{
	for(const auto& obstacle : editorObstacles)
	{
		   ...
		   tileY >= 0 && tileY < VISIBLE_MAP_HEIGHT)
		{
			...
			HPEN obstaclePen = CreatePen(PS_SOLID,2,RGB(200,0,0)); // 굵은 빨간색 테두리 추가
			...
			// 방향 표시를 더 눈에 띄게 조정
			POINT arrowPoints[3];
			int arrowSize = TILE_SIZE / 2; // 화살표 크기 증가
...
void Editor::PlaceTile(int x,int y)
{
	// 가장자리 타일은 변경 불가
...
void Editor::PlaceStartPoint(int x,int y)
{
	// 해당 위치에 스프라이트가 있는지 확인
	if(FindSprite(x,y) >= 0)
	{
		return;
	}
...
void Editor::PlaceItem(int x,int y)
{
	if(FindSprite(x,y) >= 0)
		return;
...
void Editor::PlaceMonster(int x,int y)
{
	if(FindSprite(x,y) >= 0)
		return;
...
void Editor::PlaceObstacle(int x,int y)
{
...
void Editor::ChangeObstacleDirection(Direction dir)
{
	selectedObstacleDir = dir;
}

void Editor::AddSprite(FPOINT position,Texture* texture,SpriteType type)
{
	Sprite newSprite;
...
void Editor::RemoveSprite(int x,int y)
{
...
int Editor::FindSprite(int x,int y)
{
...
void Editor::AddObstacle(POINT position,Texture* texture,Direction dir)
{
...
void Editor::RemoveObstacle(int x,int y)
{
...
int Editor::FindObstacle(int x,int y)
{
...
void Editor::ClearMap()
{
	for(int y = 0; y < VISIBLE_MAP_HEIGHT; y++)
	{
		for(int x = 0; x < VISIBLE_MAP_WIDTH; x++)
		{
			// 가장자리인 경우 무조건 wall 타일로 설정하고 변경 불가능하게 유지
			if(x == 0 || y == 0 || x == VISIBLE_MAP_WIDTH - 1 || y == VISIBLE_MAP_HEIGHT - 1)
			{
				mapTiles[y][x].tileIndex = 4; // Wall 타일 인덱스
				mapTiles[y][x].type = RoomType::WALL;
			}
			else // 내부 타일만 초기화
			{
				mapTiles[y][x].tileIndex = 10; // Floor 타일 인덱스
				mapTiles[y][x].type = RoomType::FLOOR;
			}
		}
	}
	...
	MessageBox(g_hWnd,TEXT("맵이 초기화되었습니다."),TEXT("알림"),MB_OK);
}

void Editor::PrepareDataForSave()
{
	// 맵 타일 데이터 변환
	vector<Room> tiles;
	for(int y = 0; y < VISIBLE_MAP_HEIGHT; y++)
...
void Editor::LoadFromDataManager()
{
	// 맵 데이터 로드
	MapData mapData;
